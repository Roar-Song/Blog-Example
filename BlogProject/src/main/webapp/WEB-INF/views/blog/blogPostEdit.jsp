...
<input type="hidden" name="post_no" id="post_no" value="${data.POST_NO}"/>
<input type="hidden" name="menu_no" id="menu_no" value="0" />
<input type="hidden" name="page" id="page" value="1" />
				<div class="write_top">
					<input type="text" name="post_title" id="post_title" placeholder="제목" class="input_uline wfull_size" value="${ data.POST_TITLE }"/>
				</div>
				<div class="write_content">
					<textarea name="post_con" id="post_con">${data.POST_CON}</textarea>
				</div>
				<div class="write_bottom">
							...
							<div class="sce_dropdown">
								<select name="cate_no" id="cate_no" class="select_normal">
									<c:if test="${ !empty menu.MENU1 }">
										<c:choose>
											<c:when test="${ data.POST_CATE eq 1 }"><option value="1" selected="selected">${ menu.MENU1 }</option></c:when>
											<c:otherwise><option value="1">${ menu.MENU1 }</option></c:otherwise>
										</c:choose>
									</c:if>
									<c:if test="${ !empty menu.MENU2 }">
										<c:choose>
											<c:when test="${ data.POST_CATE eq 2 }"><option value="2" selected="selected">${ menu.MENU2 }</option></c:when>
											<c:otherwise><option value="2">${ menu.MENU2 }</option></c:otherwise>
										</c:choose>
									</c:if>
									<c:if test="${ !empty menu.MENU3 }">
										<c:choose>
											<c:when test="${ data.POST_CATE eq 3 }"><option value="3" selected="selected">${ menu.MENU3 }</option></c:when>
											<c:otherwise><option value="3">${ menu.MENU3 }</option></c:otherwise>
										</c:choose>
									</c:if>
									<c:if test="${ !empty menu.MENU4 }">
										<c:choose>
											<c:when test="${ data.POST_CATE eq 4 }"><option value="4" selected="selected">${ menu.MENU4 }</option></c:when>
											<c:otherwise><option value="4">${ menu.MENU4 }</option></c:otherwise>
										</c:choose>
									</c:if>
									<c:if test="${ !empty menu.MENU5 }">
										<c:choose>
											<c:when test="${ data.POST_CATE eq 5 }"><option value="5" selected="selected">${ menu.MENU5 }</option></c:when>
											<c:otherwise><option value="5">${ menu.MENU5 }</option></c:otherwise>
										</c:choose>
									</c:if>
								</select>
							</div>
					...
					<div class="btn_black w60_30_size" id="btnUpdate">저장</div>
